...
export default function UploadPage() {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    if (!name.trim()) { setError("Enter a dataset name"); return; }
    if (!file) { setError("Select a CSV file"); return; }
    if (!year) { setError("Enter the inspection year"); return; }
  ...
  return (
    <>
      <TopBar
        ...
        subtitle="Upload an ILI inspection CSV to store in MongoDB"
      />
      ...
      <div className="mx-auto max-w-3xl space-y-6 p-6">
        <form onSubmit={handleUpload} className="space-y-4">
          ...
          <Card title="Data File">
            <FileUpload label="Upload CSV or Excel" value={file} onChange={setFile} accept=".csv,.xlsx,.xls" />
          </Card>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-text-muted">
                    {success.row_count} rows &middot; {success.columns.length} columns &middot; ID: {success.id}
                  </p>
                </div>
              </div>
            </Card>
