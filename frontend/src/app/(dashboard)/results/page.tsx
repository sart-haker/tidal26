  ...
  return (
    <>
      <TopBar
        ...
        subtitle={`${run1} vs ${run2} Inspection Comparison`}
        actions={
          <Select
            options={pairOptions}
            value={selectedPair}
      ...
      <div className="p-6 space-y-6">
        ...
        {activeTab === "exceptions" && <ExceptionsTab data={data} stats={stats} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ChartCard title="Anomaly Types" subtitle="Matched anomalies by type">
          <BarChart
            ...
            xKey="type"
            series={[{ key: "count", label: "Count", color: "#7C5CFC" }]}
          />
        </ChartCard>
        ...
        <ChartCard title="Weld Offset Progression" subtitle="Position correction along pipeline">
          <LineChart
            data={weldOffsets as unknown as Record<string, unknown>[]}
            xKey="distance"
            series={[{ key: "offset", label: "Offset (ft)", color: "#3B82F6" }]}
          />
        </ChartCard>
      </div>
...
function MatchedTab({ data }: { data: ReturnType<typeof getRunData> }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <ChartCard title="Growth Rate Distribution" subtitle="Depth growth rates (%/yr)">
          <Histogram data={growthRates} xLabel="Growth Rate (%/yr)" yLabel="Count" bins={20} />
        </ChartCard>

        <ChartCard title="Confidence Distribution" subtitle="Match confidence scores">
          <Histogram
            data={confidences}
            xLabel="Confidence"
            yLabel="Count"
      ...
      <Card title="Matched Anomalies" subtitle="All matched anomaly pairs with growth data">
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <h3 className="text-base font-semibold text-text-primary">New Anomalies</h3>
          <Badge variant="new">{stats.newCount}</Badge>
        </div>
        <p className="text-sm text-text-muted">
          Anomalies found in the later run that were not present in the earlier run.
        </p>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <h3 className="text-base font-semibold text-text-primary">Missing Anomalies</h3>
          <Badge variant="missing">{stats.missingCount}</Badge>
        </div>
        <p className="text-sm text-text-muted">
          Anomalies from the earlier run that were not found in the later run (possibly repaired or
          re-classified).
        </p>
...
function ClustersTab({ run2 }: { run2: number }) {
  ...
  const avgSeverity =
    clusters.length > 0
      ? Math.round(
          (clusters.reduce((s, c) => s + c.severity_score, 0) / clusters.length) * 10
        ) / 10
      : 0;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <AreaChart
            ...
            xKey="distance"
            series={[
              { key: "max_probability", label: "Max", color: "#EF4444" },
              { key: "avg_probability", label: "Average", color: "#F59E0B" },
            ]}
          />
        </ChartCard>
